...
<style>
    ...
    .list-ipa { font-size: 13px; color: #888; font-family: monospace; }
    ...
    /* 按鈕微調 */
    .stButton>button { 
        border-radius: 8px; font-weight: bold; width: 100%; min-height: 38px; height: 38px;
    ...
    a.custom-link-btn {
        ...
        border: 1px solid #d6d6d8; font-weight: 600; font-size: 12px;
...
def download_page():
    ...
    if not user_df.empty:
        ...
        # MP3 (恢復)
        st.markdown("---")
        st.caption("製作 MP3 (最多50字，依目前排序)")
        if st.button("🎵 製作 MP3", key="gen_mp3", use_container_width=True):
            with st.spinner("錄音中..."):
...
def main_page():
    ...
    with tabs[2]:
        if not st.session_state.is_sliding:
            if st.button("▶️ 開始輪播", type="primary", use_container_width=True): st.session_state.is_sliding = True; safe_rerun()
        else:
            if st.button("⏹️ 停止輪播", type="primary", use_container_width=True): st.session_state.is_sliding = False; safe_rerun()
